...
The workspace verifies JWTs locally (HS256 via shared secret, RS256 via
...
turn an Identity outage into a workspace outage, which is worse for most
deployments.
"""
...
class RevocationCache:
    ...
    async def _poll_once(self) -> None:
        ...
        try:
            ...
            # The server returns ``next_since`` even on empty pages; fall
            # back to the existing cursor if it's missing or non-numeric.
            try:
    ...
    async def start(self) -> None:
        ...
        the first request hits the auth middleware. Subsequent ticks run
        in a background task; cancelling it (via :meth:`stop`) is safe.
        """
        ...
        # Initial fetch synchronously so the first verify after startup
        ...
        self._task = asyncio.create_task(
            _loop(), name="plinth-workspace-revocation-poll"
        )
