...
## Quality verification — prove shaping never changes the answer

`plinth_proxy.verify` replays every (tool response, question) pair through a
model twice — once with the **raw** payload, once with the **shaped** one —
and diffs the answers. Two numbers come out: **% tokens saved** and
**% answers unchanged**, plus the specific regressions (tool + question +
both answers), so an over-aggressive keep-list is caught before it ships.

```bash
cd services/proxy
python -m plinth_proxy.verify examples/verify/orders.spec.yaml \
    --base-url http://localhost:11434/v1 --model llama3.1 \
    [--judge-model llama3.1]   # optional tiebreak for differently-worded answers
```

Exit code 0 = every answer equivalent (tune with `--fail-under`). `--json`
emits a machine-readable report. Works against any OpenAI-compatible
endpoint (OpenAI, Ollama, vLLM — or the proxy itself). This harness is
offline tooling; the request path stays a pure, deterministic transform.

The public savings preview on the landing page (`/preview`) uses a
client-side TypeScript port of the shaping engine. Parity with this
package is enforced by `landing/tests/parity.test.ts` against fixtures
generated by `scripts/gen_shaper_fixtures.py` — regenerate them whenever
the lossless layer or projection behaviour changes.
