...
| | |
|--|--|
| **Latest milestone** | **v1.5.0 — Tier-2 Sweep (Studio, 3 OAuth, Go + Swift + Kotlin SDKs)** |
| **Repo size** | ~140,000 lines of source + docs |
| **Tests passing** | **~2,867** across 7 SDKs |
...
| **Coordination** | memory (default) or Redis (cluster-shared) |
...
| v0.3 | Production-credible (real auth, multi-tenancy, GitHub OAuth, TS-SDK parity) | done |
| v0.4 | Scale & operability (Postgres, OTel export, RS256 with rotation, more OAuth) | done |
| v0.5 | Reliability & coordination depth (migrations, durable workers, transactions, typed channels, benchmarks) | done |
| v0.6 | Distribution & polish (federated revocation, migration rollback, workflow viz, generic locks) | done |
| **v1.0** | **General Availability (per-tenant quotas, GDPR, tamper-evident audit, k8s/Helm/Terraform, SLOs, threat model)** | **shipped** |
| v1.5 | Tier-2 Sweep (Studio, 3 OAuth, Go + Swift + Kotlin SDKs) | current |
| post-1.5 | Continuous improvement (OTel migration, federated revocation, advisory locks, enterprise SSO, compliance certs) | next |

### Backwards compatibility

Every demo from v0.1 forward still runs unchanged. New features are opt-in by default. **API v1 is stable** — additive-only changes guaranteed for ≥12 months; breaking changes require a `/v2/` namespace with deprecation headers and migration guide.
...
1. **The agent calls the SDK.** It says things like *"store this fact under key X"* or *"call this external tool with these arguments."*
...
4. **The Tool Gateway** is smarter than a plain proxy:
   - If the same tool was called with the same arguments recently → returns the cached result instantly.
   - Otherwise → applies authentication, checks the agent's rate limit and cost ceiling, then calls the actual external tool.
...
5. **Multi-agent coordination** uses a separate primitive called *channels*. When agent A finishes its part, it drops a typed message in a channel. Agent B picks it up — but only that message, not the entire conversation history that produced it.
...
make test           # ~2,867 tests run in seconds
...
make demo-triage    # GitHub-issue triage agent
open http://localhost:7424/   # web dashboard
```
