...
// Plynf proxy client for the Teams bot. Mirrors the Slack client.
...
export function isKnownTool(name) { return TOOLS.has(name); }
export function listTools() { return [...TOOLS]; }

export async function shapeTool({
  baseUrl, apiKey, tool, args, agentId = 'teams-bot', workflowId,
}) {
...
export async function chat({ baseUrl, apiKey, model, systemPrompt, userPrompt, toolJson }) {
