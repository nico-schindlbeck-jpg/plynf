#!/bin/sh
# Plynf installer — Stufe 1 (one-liner)
# Usage:
...
banner() {
    cat <<EOF
${C_BOLD}Plynf installer${C_RESET}
${C_DIM}─────────────────${C_RESET}
...
detect_platform() {
    ...
    case "$OS" in
        ...
        *)      die "unsupported OS '$OS' — Plynf installs on macOS and Linux only." ;;
    esac
...
stop_existing() {
    if [ -x "$PLINTH_BIN_DIR/plinth" ]; then
        debug "Stopping any running Plynf services..."
...
install_systemd() {
    ...
    if has loginctl && ! loginctl show-user "$USER" 2>/dev/null | grep -Fq 'Linger=yes'; then
        warn "  Tip: to keep Plynf running across reboots/logouts, run:"
...
${C_GREEN}✔ Plynf is installed.${C_RESET}
