...
"""Canned fixture content for the Mock MCP Server.

The fixture data here MUST stay byte-identical to the bundled fixtures in
``examples/01-research-agent/shared.py`` so the research-agent demo behaves
the same whether it pulls sources from the mock-mcp server or from the
in-process bundle.

Each topic exposes 5 sources at ``mock://<topic-slug>-<n>`` for n in 1..5.
"""
...
RENEWABLE_SOURCES: list[dict[str, str]] = [
    {
...
AI_AGENTS_SOURCES: list[dict[str, str]] = [
    {
...
CLIMATE_POLICY_SOURCES: list[dict[str, str]] = [
    {
...
FIXTURES: dict[str, list[dict[str, str]]] = {
    "renewable energy": RENEWABLE_SOURCES,
    "ai agents": AI_AGENTS_SOURCES,
    "climate policy": CLIMATE_POLICY_SOURCES,
}


# Map every fixture URL straight to its source dict for O(1) lookup.
URL_INDEX: dict[str, dict[str, str]] = {
    src["url"]: src
    for sources in FIXTURES.values()
    for src in sources
}
...
def slugify(value: str) -> str:
    ...
    Mirrors ``examples/01-research-agent/shared.py:slugify`` so any URL the
    demo invents is also recognised by the mock server.
    """
...
def get_fixture_sources(topic: str) -> list[dict[str, str]]:
    """Return the 5 fixture sources for a topic.

    Falls back to the renewable-energy fixtures for unknown topics so the
    demo always produces a meaningful comparison even with user-specified
    topics that we don't have canned data for.
    """
    canonical = topic.strip().lower()
    if canonical in FIXTURES:
        return FIXTURES[canonical]
    for key, sources in FIXTURES.items():
        if key in canonical or canonical in key:
            return sources
    base = FIXTURES["renewable energy"]
    return [
        {
            "url": f"mock://{slugify(topic)}-{i + 1}",
            "title": (
                f"{topic.title()}: "
                f"{src['title'].split(':', 1)[-1].strip() if ':' in src['title'] else src['title']}"
            ),
            "snippet": src["snippet"],
            "content": src["content"],
        }
...
def lookup_url(url: str) -> dict[str, str] | None:
    """Return the fixture entry for a ``mock://`` URL or ``None`` if unknown."""
