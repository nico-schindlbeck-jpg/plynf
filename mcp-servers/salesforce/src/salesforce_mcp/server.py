...
"""FastAPI app for the Salesforce MCP server."""
...
def create_app(settings: Settings | None = None) -> FastAPI:
    """Build a FastAPI app for the Salesforce MCP server."""
    if settings is None:
    ...
    async def lifespan(app: FastAPI):
        async with httpx.AsyncClient() as client:
            ...
            log.info(
                "salesforce-mcp.startup",
                port=settings.port,
                api_version=settings.api_version,
                tools=len(TOOL_LIST),
            )
            yield
        log.info("salesforce-mcp.shutdown")

    app = FastAPI(
        title="Plinth Salesforce MCP Server",
        version=__version__,
        lifespan=lifespan,
    )

    metrics = MetricsRegistry(service_name="salesforce-mcp", version=__version__)
    ...
    async def healthz() -> dict[str, str]:
        return {"status": "ok", "version": __version__, "service": "salesforce-mcp"}
    ...
    async def invoke(
        ...
        x_plinth_oauth_instanceurl: str | None = Header(default=None),
    ) -> JSONResponse:
        ...
        instance_url = (x_plinth_oauth_instanceurl or "").strip() or None
        ctx = ToolContext(
            access_token=token,
        ...
        log.info(
            "salesforce-mcp.invoke",
            tool_id=tool_name,
            has_token=bool(token),
            has_instance_url=bool(instance_url),
        )
        registry: MetricsRegistry = request.app.state.metrics
        ...
        try:
            ...
            log.exception("salesforce-mcp.invoke.unexpected", tool_id=tool_name)
            _record_mcp_invocation(registry, tool_name, start_t, ok=False)
            return JSONResponse(
                status_code=500,
                content=_build_error(
