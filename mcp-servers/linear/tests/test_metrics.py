...
"""Tests for the linear-mcp Prometheus metrics surface.

Mirrors the other MCP servers' metrics tests — the surface is identical
...
from linear_mcp.metrics import MetricsRegistry
from linear_mcp.server import _record_mcp_invocation
...
async def test_metrics_endpoint_includes_build_info(
    ...
    assert 'service="linear-mcp"' in line
...
def test_record_mcp_invocation_ok() -> None:
    registry = MetricsRegistry("linear-mcp", "test")
    ...
    _record_mcp_invocation(registry, "issues.create", 0.0, ok=True)
    body = registry.render()
    assert 'plinth_mcp_invocations_total{result="ok",tool="issues.create"} 1' in body
...
def test_record_mcp_invocation_error_increments_error_counter() -> None:
    ...
    _record_mcp_invocation(registry, "issues.list", 0.0, ok=False)
    body = registry.render()
    assert (
        'plinth_mcp_invocations_total{result="error",tool="issues.list"} 1'
        in body
    )
    assert 'plinth_mcp_invocation_errors_total{tool="issues.list"} 1' in body
