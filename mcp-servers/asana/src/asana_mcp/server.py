...
"""FastAPI app for the Asana MCP server."""
...
def create_app(settings: Settings | None = None) -> FastAPI:
    """Build a FastAPI app for the Asana MCP server."""
    if settings is None:
    ...
    async def lifespan(app: FastAPI):
        async with httpx.AsyncClient() as client:
            ...
            yield
        log.info("asana-mcp.shutdown")

    app = FastAPI(
        title="Plinth Asana MCP Server",
        version=__version__,
        lifespan=lifespan,
    )

    metrics = MetricsRegistry(service_name="asana-mcp", version=__version__)
    ...
    async def healthz() -> dict[str, str]:
        return {"status": "ok", "version": __version__, "service": "asana-mcp"}
    ...
    async def invoke(
        ...
        log.info("asana-mcp.invoke", tool_id=tool_name, has_token=bool(token))
        registry: MetricsRegistry = request.app.state.metrics
        ...
        try:
            ...
            log.exception("asana-mcp.invoke.unexpected", tool_id=tool_name)
            _record_mcp_invocation(registry, tool_name, start_t, ok=False)
            return JSONResponse(
                status_code=500,
                content=_build_error(
