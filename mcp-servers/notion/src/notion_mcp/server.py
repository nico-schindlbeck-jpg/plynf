...
"""FastAPI app for the Notion MCP server.

Exposes:

* ``GET /healthz`` — liveness/version probe.
* ``GET /tools`` — full tool registry metadata.
* ``POST /invoke/{tool_name}`` — dispatch a tool call. Reads the Notion access
...
:mod:`notion_mcp.tools`, and surfaces errors via the canonical Plinth error
...
def _record_mcp_invocation(
    ...
    *,
    ...
    """Record a Notion MCP invocation outcome on the registry.
...
def create_app(settings: Settings | None = None) -> FastAPI:
    """Build a FastAPI app for the Notion MCP server.
    ...
    async def lifespan(app: FastAPI):
        async with httpx.AsyncClient() as client:
            ...
            yield
        log.info("notion-mcp.shutdown")

    app = FastAPI(
        title="Plinth Notion MCP Server",
        version=__version__,
        lifespan=lifespan,
    )

    # v1.1 — Prometheus metrics. Pre-declared so a fresh server returns the
    # canonical schema even before any tool has been invoked.
    metrics = MetricsRegistry(service_name="notion-mcp", version=__version__)
    ...
    async def healthz() -> dict[str, str]:
        return {"status": "ok", "version": __version__, "service": "notion-mcp"}
    ...
    async def invoke(
        ...
        log.info("notion-mcp.invoke", tool_id=tool_name, has_token=bool(token))
        registry: MetricsRegistry = request.app.state.metrics
        ...
        try:
            ...
            log.exception("notion-mcp.invoke.unexpected", tool_id=tool_name)
            _record_mcp_invocation(registry, tool_name, start_t, ok=False)
            return JSONResponse(
                status_code=500,
                content=_build_error(
