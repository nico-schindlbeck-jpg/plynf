...
"""FastAPI app for the Google Workspace MCP server.

Exposes:

* ``GET /healthz`` — liveness/version probe.
* ``GET /tools`` — full tool registry metadata.
* ``POST /invoke/{tool_name}`` — dispatch a tool call. Reads the Google
  access token from the inbound ``Authorization: Bearer ...`` header
  (forwarded by the gateway).
* ``GET /metrics`` — Prometheus exposition.

This server is intentionally thin: it parses the auth header, dispatches to
:mod:`google_workspace_mcp.tools`, and surfaces errors via the canonical
Plinth error envelope. All input validation and HTTP plumbing live in the
tools module.
"""
...
def _record_mcp_invocation(
    ...
    *,
    ...
    """Record a Google Workspace MCP invocation outcome on the registry."""
    if registry is None:
        return
    try:
...
def create_app(settings: Settings | None = None) -> FastAPI:
    """Build a FastAPI app for the Google Workspace MCP server.
    ...
    async def lifespan(app: FastAPI):
        async with httpx.AsyncClient() as client:
            ...
            log.info(
                "google-workspace-mcp.startup",
                port=settings.port,
                drive_base_url=settings.drive_base_url,
                tools=len(TOOL_LIST),
            )
            yield
        log.info("google-workspace-mcp.shutdown")

    app = FastAPI(
        title="Plinth Google Workspace MCP Server",
        version=__version__,
        lifespan=lifespan,
    )

    metrics = MetricsRegistry(service_name="google-workspace-mcp", version=__version__)
    ...
    async def healthz() -> dict[str, str]:
        return {"status": "ok", "version": __version__, "service": "google-workspace-mcp"}
    ...
    async def invoke(
        ...
        ctx = ToolContext(
            ...
            drive_base_url=s.drive_base_url,
            docs_base_url=s.docs_base_url,
            sheets_base_url=s.sheets_base_url,
            gmail_base_url=s.gmail_base_url,
            calendar_base_url=s.calendar_base_url,
            timeout=s.request_timeout_seconds,
        )

        log.info("google-workspace-mcp.invoke", tool_id=tool_name, has_token=bool(token))
        registry: MetricsRegistry = request.app.state.metrics
        ...
        try:
            ...
            log.warning(
                "google-workspace-mcp.invoke.error",
                tool_id=tool_name,
                code=exc.code,
                message=exc.message,
            )
            ...
            log.exception("google-workspace-mcp.invoke.unexpected", tool_id=tool_name)
            _record_mcp_invocation(registry, tool_name, start_t, ok=False)
            return JSONResponse(
                status_code=500,
                content=_build_error(
