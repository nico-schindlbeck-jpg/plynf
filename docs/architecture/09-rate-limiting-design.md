...
- **Token-aware limits for model-call cost.** The same bucket model could throttle agent **model** spend if model calls flowed through a Plinth proxy. Today they don't (per arch doc 01 §1, model calls bypass Plinth). v0.4+ if/when we add a model-side gateway.
